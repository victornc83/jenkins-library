...
def call(project, app){
  loginOpenshift(project){
    ...
    def host = sh script: "oc get svc ${app} --template='{{.spec.clusterIP}}'",returnOutput: true
    def port = sh script: "oc get svc ${app} --template='{{range .spec.ports}}{{.port}}{{end}}'",returnOutput: true
